...
#from yolo3.model2 import preprocess_true_boxes, yolo_body, tiny_yolo_body, yolo_loss, yolo_eval_v2
...
def cal_lr(step,end,default_rate):
    if step>end*0.9:
        return default_rate*0.1
    elif step>end*0.8:
        return default_rate*0.3
    elif step>end*0.7:
        return default_rate*0.7
    else:
        return default_rate
...
parser.add_argument('--model', type=str, help='The .txt file include <img path>, <bbox>, <class>',
                    default='yolo_logs/models/Step1_yolo_weight_mAP_best.h5')
...
os.makedirs(os.path.join(LOGS_PATH, now))
...
class Yolo(object):
    def __init__(self):
        ...
        # training batch size
        self.step1_batch_size = batch_size_1#32*gpu_counts
        self.step2_batch_size = batch_size_2#16*gpu_counts  # note that more GPU memory is required after unfreezing the body
        ...
        if self.is_tiny_version:
            ...
            #self.yolo_model, self.yolo_body = self.create_model_tiny(yolo_weights_path='yolo_logs/models/Step1_yolo_weight_loss_best.h5')
        else:
            self.yolo_model, self.yolo_body = self.create_model(yolo_weights_path=args.model)
            #self.yolo_model, self.yolo_body = self.create_model(yolo_weights_path='yolo_logs/models/Step1_yolo_weight_loss_best.h5')
        ...
        # Temp file path
        try:
            shutil.rmtree("tmp_gt_files")
            shutil.rmtree("tmp_pred_files")
            shutil.rmtree("tmp_gt_files_org")
        except:
            pass
        ...
        self.callback = TensorBoard(os.path.join(LOGS_PATH, now))
    ...
    def train(self):
        ...
        loss_maximum = 0
        start = 0
        end = epoch_cnt1 * epoch
        loss_save_dict = OrderedDict()
        mAP_save_dict = OrderedDict()
        # Yolo Compile
        #-----
        #self.yolo_model=multi_gpu_model(self.yolo_model,gpus=2)
        #-----
        plot_model(self.yolo_model,to_file="model.png")
        #self.yolo_model.compile(loss={'yolo_loss': lambda y_true, y_pred: y_pred})
        self.yolo_model.compile(loss={'yolo_loss': lambda y_true, y_pred: y_pred}, optimizer=Adam(lr=lr_1))
        for step in range(start, end):
            # ---------------------
            #      Train Yolo
            # ---------------------
            img_input, y_true = next(yolo_train_batch)
            rate=cal_lr(step,end,lr_1)
            K.set_value(self.yolo_model.optimizer.lr,rate)
            y_loss = self.yolo_model.train_on_batch([img_input, *y_true], dummy_r)
            print("%d [Yolo step1 loss: %f]" % (step, y_loss),end,epoch,rate)
        ...
        start = end
        end = end + epoch_cnt2 * epoch
        self.set_trainability(self.yolo_model, trainable=True)
        #self.yolo_model=multi_gpu_model(self.yolo_model,gpus=2)
        self.yolo_model.compile(loss={'yolo_loss': lambda y_true, y_pred: y_pred}, optimizer=Adam(lr=lr_2))
        for step in range(start, end):
            # ---------------------
            #      Train Yolo
            # ---------------------
            img_input, y_true = next(yolo_train_batch)
            rate=cal_lr(step,end,lr_2)
            K.set_value(self.yolo_model.optimizer.lr,rate)
            y_loss = self.yolo_model.train_on_batch([img_input, *y_true], dummy_r)
            print("%d [Yolo step2 loss: %f]" % (step, y_loss),end,epoch)
